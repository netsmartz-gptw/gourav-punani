...
const { createGoalController } = require("../../src/controller");
...
describe("create goal :should respond with 400 status code", () => {
    const childUid = process.env.JEST_CHILD_UID
    test("when access token is missing", async () => {
        const response =  await request.post("/galileo/goal").set('accesstoken','').set('childuid',childUid).send({
            title: "Goal test 8",
            goalAmount: 30,
            weeklyAllocation: 10
        })
    ...
    test("weekly allocation can't be greater than 100", async () => {
        const response =  await request.post("/galileo/goal").set('accesstoken',accessToken).set('childuid',childUid).send({
            title: "Goal test 8",
            goalAmount: 30,
...
describe("create goal: should respond with 403 status code", () => {
    test("without child uid", async () => {
        const response =  await request.post("/galileo/goal").set("accesstoken", accessToken).send({
            title: "Goal test 8",
            goalAmount: 30,
            weeklyAllocation: 10
        })
...
describe("create goal : should respond with 200 status code", () => {
