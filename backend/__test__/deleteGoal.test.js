...
beforeAll(async () => {
  ...
  describe("should respond with 400 status code", () => {
	test("when access token is missing", async () => {
		const response =  await request.delete("/galileo/goal").params({id : ''}).set('accesstoken','');
...
describe("parent test cases", () => {
	test("Parent's access token without childuid: should respond with 403 status code", async () => {
		const response =  await request.delete("/galileo/goal").params({id : ''}).set('accesstoken', accessToken);
    ...
    test(" VALID childuid: should respond with 200 status code", async () => {
		...
		const response =  await request.delete("/galileo/goal").params({id : '1'}).set('accesstoken', accessToken).set('childUid', childUid);
    ...
    test(" VALID childuid with deleted id: should respond with 400 status code", async () => {
		const childUid = process.env.JEST_COGNITO_CHILD_UID;
		const response =  await request.delete("/galileo/goal").params({id : '1'}).set('accesstoken', accessToken).set('childUid', childUid);
		expect(response.statusCode).toBe(400)
		expect(response.headers['content-type']).toEqual(expect.stringContaining("json"))
